...
int main(int argc, char **argv)
{
    ...
    avr_irq_t *rx = avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('1'), UART_IRQ_INPUT);
    avr_irq_t *tx = avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('1'), UART_IRQ_OUTPUT);
    avr_irq_t *xon = avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('1'), UART_IRQ_OUT_XON);
    if (!rx || !tx) return 2;
    ...
    if (sent != sizeof(message)-1 || echoed_len != sizeof(echoed) ||
        ...
        fprintf(stderr, "USART1 IRQ RING FAIL: sent=%zu echoed=%zu\n", sent, echoed_len);
        return 1;
    }
    puts("USART1 IRQ RING PASS: 32-byte interrupt loopback");
    return 0;
}
