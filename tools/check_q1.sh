...
for tool in avr-gcc avr-gdb avr-objdump avr-nm simavr timeout; do
...
probe_eeprom() {
    elf="$1"
    log="$elf.eeprom.gdb.log"

    simavr -m atmega1284p -f 8000000 -g "$elf" >"$elf.eeprom.sim.log" 2>&1 &
    sim_pid=$!
    trap 'kill "$sim_pid" 2>/dev/null || true' EXIT INT TERM
    sleep 1
    ...
    timeout 10s avr-gdb -q -batch "$elf" \
        -ex "target remote :1234" \
        -ex "break eeprom_ready" \
        -ex "continue" \
        -ex "p/x *(unsigned char*)&eeprom_address" \
        -ex "p/x *(unsigned char*)&eeprom_written" \
        -ex "p/x *(unsigned char*)&eeprom_readback" \
        -ex "quit" >"$log" 2>&1
    rc=$?
    set -e
    ...
    test "$rc" -eq 0 || { cat "$log" >&2; fail "GDB EEPROM probe failed for $elf"; }
    values=$(awk '/^[$][0-9]+ = 0x/ { sub(/^.*= /, ""); print }' "$log")
    set -- $values
    test "$#" -ge 3 || { cat "$log" >&2; fail "could not read EEPROM probe values from $elf"; }
    test "$1" = "0x12" || fail "unexpected EEPROM address in $elf: $1"
    test "$2" = "0x5a" || fail "unexpected EEPROM write value in $elf: $2"
    test "$3" = "0x5a" || fail "EEPROM write/read mismatch in $elf: readback=$3"
}
...
probe_resource_budget() {
    elf="$1"
    log="$elf.budget.gdb.log"
    simavr -m atmega1284p -f 8000000 -g "$elf" >"$elf.budget.sim.log" 2>&1 &
    sim_pid=$!
    trap 'kill "$sim_pid" 2>/dev/null || true' EXIT INT TERM
    sleep 1
    set +e
    timeout 10s avr-gdb -q -batch "$elf" \
        ...
        -ex "continue" \
        -ex "p/x *(unsigned char*)&budget_result" \
        -ex "p/x *((unsigned char*)&budget_sp_before+0)" \
        -ex "p/x *((unsigned char*)&budget_sp_before+1)" \
        -ex "p/x *((unsigned char*)&budget_sp_deep+0)" \
        -ex "p/x *((unsigned char*)&budget_sp_deep+1)" \
        -ex "p/x *((unsigned char*)&budget_sp_after+0)" \
        -ex "p/x *((unsigned char*)&budget_sp_after+1)" \
        -ex "quit" >"$log" 2>&1
    rc=$?
    set -e
    kill "$sim_pid" 2>/dev/null || true
