...
probe_resource_budget() {
    ...
    timeout 10s avr-gdb -q -batch "$elf" \
        ...
        -ex "p/x *(unsigned short*)&budget_sp_before" \
    ...
    values=$(awk '/^[$][0-9]+ = 0x/ { sub(/^.*= /, ""); print }' "$log")
    set -- $values
    test "$#" -ge 6 || { cat "$log" >&2; fail "could not read resource-budget values from $elf"; }
    ...
    start_dec=$(printf "%d" "$5"); end_dec=$(printf "%d" "$6")
    test $((end_dec-start_dec)) -eq 16 || fail "static SRAM object is not 16 bytes in $elf"
}
