...
static const uint8_t message[] = "USART1!";
...
int main(int argc, char **argv)
{
    ...
    rx = avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('1'), UART_IRQ_INPUT);
    tx = avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('1'), UART_IRQ_OUTPUT);
    xon = avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('1'), UART_IRQ_OUT_XON);
    if (!rx || !tx) {
        fprintf(stderr, "USART1 IRQs unavailable\n");
        return 2;
    }
    ...
    if (sent != sizeof(message) - 1 || echoed_len != sizeof(echoed) ||
        ...
        fprintf(stderr, "USART1 loopback mismatch: sent=%zu echoed=%zu\n", sent, echoed_len);
        return 1;
    }

    puts("USART1 LOOPBACK PASS: USART1!");
    return 0;
}
